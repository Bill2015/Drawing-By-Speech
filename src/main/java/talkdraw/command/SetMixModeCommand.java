...
/**【指令】 《切換模式指令》
    * <p>切換為人臉模式</p>*/
public class SetMixModeCommand extends Command{

    /** 建構子 */
    public SetMixModeCommand(App APP, CommandAttribute yamlLoader){
    ...
    public NextCommand execute( String ... args ) {
        ...
        switch ( phase ) {
            case None:
                return (APP.MENU_BAR.setMode( LocalConnection.USED_MODE.混合 ) ? success() : failure());
            //------------------------
            default:break;
        }
    ...
    @Override protected NextCommand success( String ... args ) {
        APP.println( "進入混合模式", App.PRINT_BOTH );
