...
/** 【指令】《網路相關指令》
    ...
    *  @param args_0 網路圖片 ID */
public class ChangeImageFromWebCommand extends Command{

    /** 建構子 */
    public ChangeImageFromWebCommand(App APP, CommandAttribute yamlLoader){
        ...
        switch ( phase ) {
            case None:
                ...
                if( APP.MAIN.isNowHaveSelected() == false ){
                    ...
                    //將目前的圖片關鍵字拿去搜尋
                    if( APP.MAIN.getNetConnection().searchImages( imgObj.getName() ) <= 0 ){
                        return failure( "未找到任何關於 ", imgObj.getName(), " 的圖片" );
                    }
                    //切換右側選單至網路圖片
                    APP.IMAGE_PANE.changeViewTabPane( ImageViewPane.NETWORK );
    ...
    private NextCommand getNetImageByID( String ID ){        
        ...
        //切換圖片
        if( !main.changeImageToLayer( imgObj, ID , Main.GET_MODE.WEB) ){
    ...
    private NextCommand getImageFromWeb( String keyword ){    
        //搜尋網路圖片
        int size = APP.MAIN.getNetConnection().searchImages( keyword );

        //更新右側的 ImageViewPane
        APP.IMAGE_PANE.updateViewBox();

        if( size <= 0 ){
            APP.println( String.join("", "沒有關於 ", keyword, " 的圖片") , App.PRINT_BOTH );
